...
def validate_submission_files(submission_dir):
    submitted = sorted(
    ...
    if missing:
        print(f"  Missing sequences: {sorted(missing)}")
    if extra:
        print(f"  Unexpected sequences: {sorted(extra)}")
    if missing or extra:
        print(f"  Expected {len(expected)} sequences, got {len(actual)}.")
        sys.exit(1)
    print(f"  All {len(submitted)} expected sequences found.")
    return submitted
...
def process_sequences(submission_dir, gt_dir, output_dir, time_tolerance=0.001):
    ...
    seq_names = validate_submission_files(submission_dir)

    for name in seq_names:
        fname = f"{name}.txt"
        sub_path = os.path.join(submission_dir, fname)
        ref_time_path = os.path.join(gt_dir, f"{name}_ref_time.txt")
        gt_path = os.path.join(gt_dir, f"{name}_gt.txt")

        # ----- Read reference timestamps -----
        if not os.path.exists(ref_time_path):
            print(f"  Reference file not found: {ref_time_path}, skipping.")
            continue
        ref_ts = np.loadtxt(ref_time_path)
        ...
        if not timestamps:
            print(f"  {fname}: empty or invalid, skipping.")
            continue
        ...
        if not (is_subset and enough):
            print(f"  {fname}: FAILED — subset={is_subset}, n_sub={len(timestamps)}, n_ref={len(ref_ts)}")
            continue
        ...
        if not os.path.exists(gt_path):
            print(f"  GT file not found: {gt_path}, skipping APE/AUC.")
            continue
        ...
        print(f"  {name}: {len(gt_ape_lines)} matched poses, {len(auc_lines)} velocity pairs")
...
def main():
    ...
    process_sequences(args.submission, args.gt, args.output)
    ...
    print("Summary")
    print("=" * 60)
    print(f"  ATE total:  {ate_total:.4f} m")
    print(f"  AUC total:  {auc_total:.6f}")
    print(f"  Scores:     {score_file}")
    print(f"  S_xi PDF:   {pdf_path}")
    print("=" * 60)
